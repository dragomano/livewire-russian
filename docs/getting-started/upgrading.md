...
### `wire:model` теперь поддерживает нотацию с квадратными скобками
...
[Подробнее о wire:model →](/html-directives/wire-model#доступ-к-вложенным-свойствам)
...
#### Удалено: внутренние события `upload:generatedSignedUrl`

Внутренний механизм загрузки файлов теперь использует единое событие `upload:plan`, решение о котором принимает сервер, вместо прежних событий `upload:generatedSignedUrl` и `upload:generatedSignedUrlForS3`. Эти события никогда не были частью публичного API, однако если ваш пользовательский JavaScript подписывался на них через `$wire.$on('upload:generatedSignedUrl', ...)`, такие обработчики перестанут вызываться.

Публичный API загрузки файлов (`Livewire.upload`, а также DOM-события `livewire-upload-start`, `livewire-upload-progress`, `livewire-upload-finish`, `livewire-upload-error` и `livewire-upload-cancel`) не изменился. Кроме того, событие прогресса теперь дополнительно содержит количество уже загруженных (`loaded`) и общее количество (`total`) байт.
...
[Подробнее о новой системе перехватчиков →](/advanced/javascript#перехватчики).
...
### Чанковые, возобновляемые и многoфайловые загрузки в S3

Теперь загрузка файлов автоматически поддерживает большие файлы и нестабильные соединения — никаких изменений в разметке или API не требуется:

- **Большие файлы обходят ограничения PHP на загрузку.** Файлы, превышающие порог чанкинга, разбиваются в браузере на части и собираются обратно на сервере, поэтому `upload_max_filesize` / `post_max_size` больше не ограничивают загрузки — ограничение задаёт ваше правило валидации `max:`.
- **Прерванные загрузки возобновляются.** Отмена, потеря соединения или полная перезагрузка страницы — при повторном выборе того же файла загрузка продолжается с тех чанков, которые сервер уже получил.
- **S3 поддерживает несколько файлов и нативный multipart.** Теперь многoфайловые загрузки в S3 работают (один подписанный PUT-запрос для каждого файла), а большие файлы используют настоящую многочастную загрузку S3.

Эта функциональность включена по умолчанию. При обновлении следует учитывать несколько моментов:

- **Чанкинг включается автоматически.** Любой локальный файл размером более 1 МБ теперь проходит через новый endpoint для загрузки чанков вместо одного POST-запроса. Установите `temporary_file_upload.chunking` в `false`, чтобы отключить это поведение.
- **Если вы задаёте собственный `middleware` загрузки,** теперь он также применяется к endpoint загрузки чанков, где один файл поступает в виде множества быстрых запросов. Строгое ограничение вроде `throttle:60,1` приведёт к сбою больших загрузок — задавайте более высокий лимит (встроенное значение по умолчанию для endpoint чанков — `throttle:600,1`).
- **Пользователям S3 следует добавить правило жизненного цикла `AbortIncompleteMultipartUpload`,** чтобы незавершённые multipart-загрузки не накапливали скрытое хранилище. Повторный запуск `php artisan livewire:configure-s3-upload-cleanup` добавит это правило автоматически.
- **`S3DoesntSupportMultipleFileUploads` больше не выбрасывается** (многoфайловые загрузки S3 теперь поддерживаются). Класс сохраняется для обратной совместимости, но помечен как устаревший; тесты, проверяющие это исключение, следует удалить.

[Подробнее о загрузках →](/features/uploads)

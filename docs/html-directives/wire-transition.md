...
### Неименованные переходы при типизированном свопе

Когда используется именованный переход (например, `slide`), Livewire воспринимает весь блок как одно целое. В этом случае любой **неименованный** элемент `wire:transition` внутри него **не становится самостоятельным** переходом. Он «едет» вместе с родительским переходом и не получает обычную браузерную анимацию (fade).

```html
<div wire:transition="slide">
    <p>Содержимое шага</p>

    {{-- Неименованный: следует за переходом родителя --}}
    <div wire:transition>
...
Если вам нужно, чтобы внутренний элемент анимировался **независимо** во время типизированного свопа, дайте ему явное имя:

```php
<div wire:transition="badge">...</div>
```

Вне типизированного перехода (при обычном обновлении DOM, например, при появлении ошибки валидации) неименованные элементы `wire:transition` продолжают работать как раньше: используют `match-element` и анимируются независимо.

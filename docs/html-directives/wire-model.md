...
Модификатор `.renderless` отправляет обновление live-модели на сервер без повторного рендеринга компонента:

```blade
<input type="text" wire:model.renderless.live="search">
```

Это полезно, когда обновление модели должно быть сохранено на сервере, но после завершения запроса не должно изменять текущий DOM.

Поскольку `.renderless` влияет только на серверные сообщения, используйте его вместе с `.live`.
...
| `.renderless` | Пропустить повторный рендеринг после обновления live-модели |

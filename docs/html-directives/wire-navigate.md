...
`wire:navigate` перехватывает только переходы на страницы того же происхождения (same-origin). Внешние ссылки, ссылки с `target`, отличным от `_self`, ссылки для скачивания, а также URL вроде `mailto:` или `tel:` автоматически обрабатываются браузером в обычном режиме.
